...
- **左侧栏（跨会话）**：实时会话卡片列表 —— 浅色 badge、缩略时间、等待中
  会话红点。底部的 `📊 会话历史` 按钮打开全局历史模态。
- **右栏（会话级 Tab）**：`📝 工作区` / `📋 AI 摘要` / `⚡ 命令` —— 只放
  当前会话的数据，和全局动作物理分开。
...
- **连接监控**：侧栏底部面板展示连接时长、重连次数、消息计数、延迟、会话数
  与当前会话状态，独立 tick，不依赖 WebSocket 心跳。
...
`📊 会话历史` 按钮；**右栏** 以 Tab 形式只展示**会话级**工作项（`📝 工作区` / `⚡ 命令`，
AI 摘要嵌在工作区里）。底部的连接监控已展开，显示连线时间 / 重连次数 / 消息数 /
延迟 / 会话数 / 当前会话状态。*
...
*从侧栏底部点 `📊 会话历史` 会打开一个应用级模态，遮罩掉会话视图，展示当日
...
| **跨会话** | 左侧边栏 | 实时会话列表 + 底部 `📊 会话历史` 按钮 → 模态 |
| **当前会话** | 右栏 Tab | `📝 工作区` / `📋 AI 摘要` / `⚡ 命令` |
...
- 🏛️ **3 层信息架构**：顶栏放应用级（设置 / 关于），侧栏放跨会话（会话列表 /
  会话历史），右栏 Tab 只放会话级（工作区 / AI 摘要 / 命令）—— 不再把全局
  动作混在每个会话的 Tab 里。

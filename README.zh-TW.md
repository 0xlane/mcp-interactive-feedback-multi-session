...
- **左側欄（跨會話）**：即時會話卡片清單 —— 淺色 badge、縮略時間、等待中
  會話紅點。底部 `📊 會話歷史` 按鈕打開全域歷史模態。
- **右欄（會話級 Tab）**：`📝 工作區` / `📋 AI 摘要` / `⚡ 命令` —— 只放
  當前會話的資料，和全域動作在物理上分開。
...
- **連線監控**：側欄底部面板顯示連線時長、重連次數、訊息計數、延遲、會話
  數與當前會話狀態，獨立 tick，不依賴 WebSocket 心跳。
...
`📊 會話歷史` 按鈕；**右欄** 以 Tab 形式只展示**會話級**工作項（`📝 工作區` / `⚡ 命令`，
AI 摘要嵌在工作區裡）。底部的連線監控已展開，顯示連線時間 / 重連次數 / 訊息數 /
延遲 / 會話數 / 當前會話狀態。*
...
*從側欄底部點 `📊 會話歷史` 會打開一個應用級模態，遮罩掉會話視圖，顯示當日
...
| **跨會話** | 左側欄 | 即時會話清單 + 底部 `📊 會話歷史` 按鈕 → 模態 |
| **當前會話** | 右欄 Tab | `📝 工作區` / `📋 AI 摘要` / `⚡ 命令` |
...
- 🏛️ **3 層資訊架構**：頂欄放應用級（設定 / 關於），側欄放跨會話（會話清單 /
  會話歷史），右欄 Tab 只放會話級（工作區 / AI 摘要 / 命令）—— 不再把全域
  動作混在每個會話的 Tab 裡。

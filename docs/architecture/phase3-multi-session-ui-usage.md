...
> | 跨会话浏览 | 左侧边栏 | 会话列表 + 底部 `🗂️ 会话历史` 按钮 → 模态（历史 + 今日统计） |
> | 当前会话 | 右侧 Tab | `📝 工作区`（AI 摘要嵌入其中）/ `⚡ 命令` |
>
...
| 侧栏底部 `🗂️ 会话历史` | `#sessionsModal` | 历史会话列表 + 今日统计 |
...
右栏 Tab 栏现在只剩 2 个真正会话级的视图：`📝 工作区`（AI 摘要嵌入其中）
...
- `📝 复制所有用户讯息`（`#copyCurrentUserContent`）

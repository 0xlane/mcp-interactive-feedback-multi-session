...
class TestI18NEnvironmentIntegration:
    ...
    def test_language_detection_in_different_environments(self):
        ...
        try:
            # 測試不同的環境設置
            # fallback_language 為 "zh-TW"（見 I18nManager._fallback_language），
            # 所以不支援的 locale（如 ja_JP）會回退到 zh-TW。
            test_cases = [
                ...
                {"LANG": "ja_JP.UTF-8", "expected": "zh-TW"},  # 不支援的語言回退到默認
            ]
    ...
    def test_i18n_language_switch_is_global(self, web_ui_manager, i18n_manager):
        """翻譯已移至前端（見 ``WebUIManager`` 注解「移除 i18n 管理器」），
        因此 WebUIManager 不再直接持有 ``i18n`` 屬性。此處驗證：
        全局 ``i18n_manager.set_language`` 切換後，再次 ``get_current_language``
        能讀到新值，保證後端仍能被前端或其他模組查詢到當前語言。
        """
        assert not hasattr(web_ui_manager, "i18n"), (
            "WebUIManager 已不再直接挂载 i18n，避免與前端翻譯重覆"
        )
        ...
        try:
            for lang in TestData.SUPPORTED_LANGUAGES:
                if lang != original_lang:
                    assert i18n_manager.set_language(lang) is True

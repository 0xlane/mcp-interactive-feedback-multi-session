...
class TestMCPBasicWorkflow:
    ...
    async def test_interactive_feedback_call_timeout(self, test_project_dir):
        ...
        try:
            ...
            # 自動化測試環境預期會超時；MCP 協議層會把超時包成
            # {'content': [{'text': '... timeout ...'}], 'isError': False}
            # 的格式，因此超時關鍵字可能出現在下列任一位置。
            timeout_texts: list[str] = []
            if "error" in result:
                timeout_texts.append(str(result["error"]))
            for item in result.get("content", []) or []:
                if isinstance(item, dict):
                    text = item.get("text")
                    if isinstance(text, str):
                        timeout_texts.append(text)

            timed_out = any(
                "超時" in t or "timeout" in t.lower() for t in timeout_texts
            )
            if timed_out:
                # 超時是預期的行為
                pass

        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.ws_connect(ws_url) as ws:
                    ...
                    # 可能會收到額外的消息（sessions_snapshot / session_updated /
                    # status_update），先處理掉以便後續測試心跳回應
                    try:
                        while True:
...
class TestWebUIErrorHandling:
    ...
    async def test_websocket_without_session(self, web_ui_manager):
        ...
        Phase 3 多會話模式：無活躍會話時連接**不應**被拒絕，瀏覽器殼可以提前
        打開並等待新會話到來。連接成功後應收到：
        - ``connection_established`` (首條，向下兼容)
        - ``sessions_snapshot`` (空 sessions 列表)
        """
        import aiohttp
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.ws_connect(ws_url) as ws:
                    # 第一條應為 connection_established
                    msg = await asyncio.wait_for(ws.receive(), timeout=5)
                    assert msg.type == aiohttp.WSMsgType.TEXT
                    data = msg.json()
                    assert data.get("type") == "connection_established"

                    # 第二條應為 sessions_snapshot，且 sessions 為空
                    msg2 = await asyncio.wait_for(ws.receive(), timeout=5)
                    assert msg2.type == aiohttp.WSMsgType.TEXT
                    data2 = msg2.json()
                    assert data2.get("type") == "sessions_snapshot"
                    assert data2.get("sessions") == []
                    assert data2.get("active_session_id") is None

            except TimeoutError:
                pytest.fail("無會話連接應立即收到 connection_established / sessions_snapshot")

...
class TestWebFeedbackSessionCleanup:
    ...
    async def test_async_cleanup(self):
        ...
        # Phase 3：清理時廣播兩個事件（notification + session_expired），
        # 都應攜帶 session_id 字段以支持多會話路由。
        assert mock_websocket.send_json.call_count >= 1
        sent_types = [
            call.args[0].get("type")
            for call in mock_websocket.send_json.call_args_list
        ]
        assert "notification" in sent_types
        assert "session_expired" in sent_types
        # 每個事件都應帶 session_id
        for call in mock_websocket.send_json.call_args_list:
            assert call.args[0].get("session_id") == self.session_id
